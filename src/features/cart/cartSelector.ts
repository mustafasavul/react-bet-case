...
export const selectTotalOdd = createSelector(selectCartItems, (items) =>
  items.length === 0 ? 0 : items.reduce((acc, item) => acc * item.oddValue, 1)
);
...
export const selectRequiredMinBetCount = createSelector(selectCartItems, (items) => {
  const defaultMbs = 3;
...
export const selectIsValid = createSelector(
  ...
  (items, stake, requiredMin) => items.length >= requiredMin && stake >= 1 && stake <= 20000
);
